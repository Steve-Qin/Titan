...
public class RestaurantMySQLConnection implements RestaurantDBConnection {
	...
	private static RestaurantMySQLConnection instance;
	private static final String CURRENT_PIPELINE = "restaurant";
	private static final int MAX_RECOMMENDED_RESTAURANTS = 10;

	
	private RestaurantMySQLConnection() {
		try {
	...
	public static RestaurantMySQLConnection  getInstance() {
		if (instance == null) {
			instance = new RestaurantMySQLConnection();
		}
		return instance;
	}
	...
	public void close() {
		if (conn != null) {
			try {
				conn.close();
			} catch (Exception expect) { /* ignored */
			}
		}
	}
	...
	public JSONArray recommendRestaurants(String userId) {
		try {
			...
			//step 1    read the history table
			Set<String> visitedRestaurants = getVisitedRestaurants(userId);
			//step 2   why hashSet? dedup by utilizing set 
	...
	public JSONArray searchRestaurants(String userId, double lat, double lon, String term) {
		try {
			...
			assert api instanceof YelpAPI : "Incompatible API";
			JSONObject response = new JSONObject(((YelpAPI) api).searchForBusinessesByLocation(lat, lon));
